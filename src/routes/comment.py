...
from services.gitlab.events.comment.comment_event_service import get_handler
...
@router.post('/')
def comment_webhook(event: CommentEvent, event_handler: Depends(get_handler), logger: Logger = Depends(resolve_logger(__name__)), authorized = Depends(require_token)):
    try:
        event_handler(event)
