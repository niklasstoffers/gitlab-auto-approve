...
stty -echo
read -rp 'Gitguardian API key: ' gg_api_key; echo
stty echo

export_global=
while ! [[ $export_global = "y" || $export_global = "N" ]]; do
    read -rp 'Do you want to add the API key as a global variable? Otherwise it has to be set each time environment variables are being reloaded [y/N]: ' export_global
done
